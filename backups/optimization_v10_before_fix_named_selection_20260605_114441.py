...
def select_named_designs(all_results, Q_in_max=1500.0, ref_losses=None, w_P=0.5):
    """
    Select the three final feasible named designs from all evaluated candidates:
    - max_power
    - max_efficiency
    - balanced

    all_results format expected:
    list of (score, params, losses)

    Final reported designs must satisfy Q_in_W <= Q_in_max.
    """
    feasible = []
    for item in all_results or []:
        if len(item) < 3:
            continue
        score, params, losses = item
        if losses is None:
            continue
        if losses.get('Q_in_W', 1e99) <= Q_in_max:
    ...
    max_power = max(feasible, key=lambda x: x[2].get('P_brake', -1e99))
    max_efficiency = max(feasible, key=lambda x: _loss_eta(x[2]))
    balanced = max(
        feasible,
        key=lambda x: _balanced_score_from_losses(x[2], ref_losses=ref_losses, w_P=w_P)
    )
