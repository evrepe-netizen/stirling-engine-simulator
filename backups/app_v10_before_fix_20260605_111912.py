...
def plot_power_efficiency_map(all_results, named_points, title="Power–Efficiency Map"):
    """
    Scatter of all evaluated designs with named points marked.
    named_points: dict keys from 'proto1','proto2','max_power','max_eta','balanced'
                  values: losses dict.
    """
    ...
    if all_results:
        xs = [l['P_brake']        for _, _, l in all_results if l]
        ys = [l['eta_brake'] * 100 for _, _, l in all_results if l]
        ax.scatter(xs, ys, s=12, color='#CCCCCC', alpha=0.5,
                   label='All candidates', zorder=2)

    style_map = {
        'proto1':    ('#1565C0', 'o', 120, 'Prototype 1'),
        'proto2':    ('#2E7D32', 's', 120, 'Prototype 2'),
        'max_power': ('#C62828', '*', 180, 'Max Power'),
        'max_eta':   ('#6A1B9A', '*', 180, 'Max Efficiency'),
        'balanced':  ('#E65100', 'D', 130, 'Balanced'),
    }
    for key, losses in named_points.items():
        if losses is None:
            continue
        color, marker, size, label = style_map.get(key, ('#333', 'o', 100, key))
        q        = losses['Q_in_W']
        feasible = q <= 1500
        edge     = 'black' if feasible else 'red'
        ax.scatter(losses['P_brake'], losses['eta_brake'] * 100,
                   s=size, color=color, marker=marker,
                   edgecolors=edge, linewidths=1.5,
                   label=label + ('' if feasible else ' ⚠️ infeasible'),
                   zorder=5)
...
with tab_schmidt:
    ...
    if sim is None:
        ...
        # 4. Diagrams — P-V (with No-Regen overlay) + P-theta
        ...
        if sim is not None:
            ...
            if sim_s_noreg is not None:
                ...
                V_nr   = (R_nr['V_e'] + R_nr['V_c']) * 1e6 + dv_nr
                ax.plot(V_nr, R_nr['P'] / 1e5, color='#FFA726', lw=2,
                        ls='--', label='No Regen')
...
with tab_both:
    ...
    if sim_s_display and sim_a:
        ...
        if sim_s_noreg is not None:
            R_nr  = sim_s_noreg['result']
            V_nr  = (R_nr['V_e'] + R_nr['V_c']) * 1e6 + _dead_vol(sim_s_noreg)
            ax.plot(V_nr, R_nr['P'] / 1e5, color='#FFA726', lw=2, ls='--',
                    label='No Regen')
        ax.set(xlabel='V_total [cm³]', ylabel='P [bar]', title='P-V Diagram — Overlay')
        ax.legend(fontsize=9); ax.grid(alpha=0.3)
...
with tab_optimize:
    ...
    # ── Stage 3 ───────────────────────────────────────────────────────────────
        ...
        obj_s3 = st.radio("Objective:", ["Max Power", "Max Efficiency", "Balanced"],
                          horizontal=True, key="obj_s3")
        ...
        if st.button("🚀 Run Stage 3 Optimization",
            ...
            method_map = {"LHS (recommended)": "lhs",
                          ...
                          "Bayesian": "bayesian"}
            try:
                bp3, bl3, feas3, raw3 = stage3_search(
                    p_s3, open_specs_s3, obj_map[obj_s3], Q_in_max_s3,
                    'schmidt', lf_s3,
                    method=method_map[method_s3],
                    n_samples=n_s3, progress_cb=cb_s3)
                prog3.empty()
                ...
                if bl3 is None:
                    ...
                    st.session_state['stage3_results'] = {
                        'best_params': bp3, 'best_losses': bl3,
                        'feasible': feas3, 'raw': raw3,
                        'objective': obj_s3, 'Q_in_max': Q_in_max_s3}

                    q3 = bl3['Q_in_W']
                    if q3 <= Q_in_max_s3:
                        st.success(f"✅ Q_in = {q3:.1f} W ≤ {Q_in_max_s3} W")
                    else:
                        st.error(f"❌ Q_in = {q3:.1f} W > {Q_in_max_s3} W")

                    ca3, cb3, cc3, cd3 = st.columns(4)
                    ca3.metric("P_brake [W]", f"{bl3['P_brake']:.2f}")
                    cb3.metric("η_net [%]",   f"{bl3['eta_brake']*100:.2f}")
                    cc3.metric("Q_in [W]",    f"{q3:.1f}")
                    cd3.metric("M [g]",       f"{bl3['M']*1000:.3f}")
            except Exception as e:
                prog3.empty()
    ...
    # ── Stage 4 ───────────────────────────────────────────────────────────────
        ...
        if st.button("🚀 Run Stage 4 (Air + Helium + Hydrogen)", key="run_s4"):
            ...
            try:
                s4_results = stage4_search(
                    proto2_params, obj_map[obj_s4],
